...
object MediaCacheService {
    ...
    private suspend fun checkReleaseStatus(
        item: MediaItem,
        data: Map<String, Any?>,
    ) {
            ...
            put(
                ...
                d.seasons.map {
                    mapOf(
                        "number" to it.number,
                        "name" to it.name,
                        "episodes" to it.episodes,
                        "posterUrl" to it.posterUrl,
                        "airDate" to it.airDate?.time,
                    )
                },
            )
            put("related", d.related.map { mapOf("id" to it.id, "title" to it.title, "posterUrl" to it.posterUrl) })
    ...
    private suspend fun fetchGame(item: MediaItem): Map<String, Any?>? {
        ...
        val igdbResult =
            if (ApiServices.igdbAvailable && externalIdInt != null && item.apiSource != "giant_bomb") {
                withContext(Dispatchers.IO) {
                ...
                if (result["platforms"] == null && gc.platforms != null) {
                    runCatching {
                        val list =
                            com.google.gson
                                .Gson()
                                .fromJson(gc.platforms, List::class.java)
                                ?.map { it.toString() }
                ...
                if (result["genre"] == null && gc.genres != null) {
                    runCatching {
                        val list =
                            com.google.gson
                                .Gson()
                                .fromJson(gc.genres, List::class.java)
                                ?.map { it.toString() }
        ...
        if (ApiServices.igdbAvailable && externalIdInt != null) {
            withContext(Dispatchers.IO) {
                ...
                fun toMaps(list: List<app.oribu.service.IgdbRelatedGame>) =
        ...
        Log.d(
            ...
            "fetchGame '${item.title}' synopsis=${(result["synopsis"] as? String)?.take(
                50,
            )} platforms=${result["platforms"]} deck=${(result["deck"] as? String)?.take(40)}",
        )
        return result
    }
    ...
    private suspend fun fetchMangaFromAniList(
        ...
        if (result["chapters"] == null) {
            ...
            val mangaDexId =
                cachedMangaDexId ?: withContext(Dispatchers.IO) {
    ...
    private suspend fun fetchBook(item: MediaItem): Map<String, Any?>? {
        ...
        val coverUrl =
            (
                imgLinks?.get("thumbnail") as? String
    ...
    private fun translateBookGenre(genre: String): String {
        ...
        val map =
            mapOf(
                "Fiction" to "Ficção",
                "Nonfiction" to "Não-ficção",
                "Non-fiction" to "Não-ficção",
                ...
                "Horror" to "Terror",
                "Romance" to "Romance",
                "Historical Fiction" to "Ficção Histórica",
                "Biography" to "Biografia",
                "Autobiography" to "Autobiografia",
                "Self-help" to "Autoajuda",
                "Psychology" to "Psicologia",
                "Philosophy" to "Filosofia",
                "History" to "História",
                "Science" to "Ciência",
                "Technology" to "Tecnologia",
                "Business" to "Negócios",
                "Economics" to "Economia",
                "Politics" to "Política",
                "Religion" to "Religião",
                "Poetry" to "Poesia",
                "Drama" to "Drama",
                "Comics" to "Quadrinhos",
                "Graphic Novels" to "Novel Gráfica",
                "Children" to "Infantil",
                "Young Adult" to "Jovem Adulto",
                "Adventure" to "Aventura",
                "Action" to "Ação",
                "Crime" to "Crime",
    ...
    private val staleReleaseMentionPattern =
        Regex(
    ...
    private fun translateGameGenres(genres: String): String {
        ...
        val map =
            mapOf(
                ...
                "Card & Board Game" to "Cartas e Tabuleiro",
                "MOBA" to "MOBA",
                "Point-and-click" to "Point-and-click",
                "Simulator" to "Simulação",
                "Sport" to "Esporte",
                "Strategy" to "Estratégia",
                "Turn-based strategy (TBS)" to "Estratégia por Turnos",
                "Tactical" to "Tático",
                "Quiz/Trivia" to "Quiz",
    ...
    private suspend fun checkSeriesStatus(
        item: MediaItem,
        data: Map<String, Any?>,
    ) {
     ...
     */
    internal fun determineSeriesStatusTransition(
